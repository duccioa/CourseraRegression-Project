...
We look therefore to other variables that might be confounder. Based on a brief literature review, the variables that can be relevant to our analysis are weight, the number of cylinders and the rear axle ratio (drat in dataset). We can call primary this variables as this are the engine and car specifications rather than performance. Other variables as "qsec" (which measures the acceleration) and "hp" (which measure the power of the engine) can be good indicators but they are performance variables and are consequences of the primary specs.
...
```
The predictor "cyl" explains `r paste(round(summary(fit_cyl)$r.squared*100, 1), "%")` of "mpg" variation. ADD DESCRIPTION OF THE SUMMARY
```{r MODEL 2}
fit_cyl_wt <- update(fit_cyl, mpg ~ cyl + wt)
```
Once we factor in the weight "wt", the model explains `r paste(round(summary(fit_cyl_wt)$r.squared*100, 1), "%")` of "mpg" variation with a RSE of `r round(summary(fit_cyl_wt)$sigma)` and very low p-values.
...
```{r MODEL 3}
fit_cyl_wt_am <- update(fit_cyl_wt, mpg ~ cyl + wt + am + wt:am)
```

Last, we add the variables "drat" and the interaction between "disp" and "cyl".

```{r MODEL 4 and 5}
fit_cyl_wt_am_drat <- update(fit_cyl_wt, mpg ~ cyl + wt + am + drat + wt:am)
fit_cyl_wt_am_drat_disp <- update(fit_cyl_wt, mpg ~ cyl + wt + am + drat + wt:am + cyl:disp)
```
We can now run an anova test to verify which of these variale can be dropped without loss of relevant information.
```{r}
anova(fit_cyl, fit_cyl_wt, fit_cyl_wt_am, fit_cyl_wt_am_drat, fit_cyl_wt_am_drat_disp)
```
...
```{r Correlation between wt and am}
attach(mtcars)
cor(wt, am)
...
```{r Correlation between disp and cyl}
attach(mtcars)
cor(cyl, disp)
